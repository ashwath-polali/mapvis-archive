...
DESCRIPTION = (
    ...
    "large and close, one island filling the entire frame: a smooth simple volcano cone "
    "of large calm rock faces with a quiet crater at the back, its slopes carpeted in low "
    "green moss and undergrowth, two colossal carved stone panther heads emerging from "
    "the volcano's slopes with jaws gaping open and one white waterfall pouring from each "
    "mouth straight down the rock into mist, one single great dark stone portal at the "
    "volcano's base, the only doorway on the whole island, clearly leading deep inside, "
    "the grand staircase descending from the portal to the harbor plaza completely clear "
    "with open flagstone on both sides, a small stone town of just a handful of varied "
    "terracotta-roofed houses, each one distinct in height and roofline, set well apart "
    "along the outer sides of the terraces with generous open flagstone between them, "
    "warm glowing windows, a few market stalls under striped awnings and some crates and "
    "barrels against the walls while every street stair and walkway stays wide open, the "
    "quay walls plain solid stone from end to end, a simple straight wooden dock at the "
    "waterfront quay, the harbor water glassy and empty, a lighthouse on a curled "
    "breakwater arm, a bare sandy cove under a bare grassy headland, the whole town "
    "silent and deserted under still clear air, long warm shadows, rich amber and deep "
    ...
    while time.time() - t0 < 900:
        ...
        if st in ("completed", "success", "succeeded", "done"):
            ...
            for k, b in enumerate(imgs):
                import base64
                p = os.path.join(OUT, f"three_{k}.png")
