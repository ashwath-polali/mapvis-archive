...
DESCRIPTION = (
    ...
    "simple straight wooden dock reaching into the glassy empty water, the beach and "
    "shallows completely bare, a port town of a handful of varied terracotta-roofed "
    "houses climbing the hillside behind the quay on natural stepped stone terraces with "
    "low stone walls no taller than a man, a few market stalls under striped awnings kept "
    "down along the waterfront quay only, a wide stone stair street winding up from the "
    "harbor between the houses to one single great dark stone portal at the volcano's "
    "base, the only doorway on the whole island, clearly leading deep inside, nothing "
    "standing on or beside the stair street its whole length, above the town a smooth "
    "simple volcano cone of large calm rock faces with a quiet crater, its slopes "
    ...
    "pouring from each mouth straight down the rock into mist, every street stair and "
    "walkway wide open and clear, the streets lit only by the warm glow of the house "
    "windows, a lighthouse on a curled breakwater arm, a bare sandy cove under a bare "
    "grassy headland, the whole town silent and deserted under still clear air, long "
    "warm shadows, rich amber and deep teal palette, clean hand-painted pixel art"
)
    ...
    while time.time() - t0 < 900:
        ...
        if st in ("completed", "success", "succeeded", "done"):
            ...
            for k, b in enumerate(imgs):
                import base64
                p = os.path.join(OUT, f"port2_{k}.png")
